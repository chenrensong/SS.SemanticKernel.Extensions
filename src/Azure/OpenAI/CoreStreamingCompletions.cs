...
{
    public class CoreStreamingCompletions : IDisposable
    {
        private readonly Response _baseResponse;
        private readonly SseReader _baseResponseReader;
        private readonly IList<CoreCompletions> _baseCompletions;
        private readonly object _baseCompletionsLock = new();
        private readonly IList<CoreStreamingChoice> _streamingChoices;
        ...
        internal CoreStreamingCompletions(Response response)
        {
            ...
            _baseCompletions = new List<CoreCompletions>();
            _streamingChoices = new List<CoreStreamingChoice>();
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    while (true)
                    {
                        ...
                        var completionsFromSse = CoreCompletions.DeserializeCompletions(sseMessageJson.RootElement);
                        ...
                        foreach (var choiceFromSse in completionsFromSse.Choices)
                        {
                            lock (_streamingChoicesLock)
                            {
                                CoreStreamingChoice existingStreamingChoice = _streamingChoices
                                    .FirstOrDefault(choice => choice.Index == choiceFromSse.Index);
                                if (existingStreamingChoice == null)
                                {
                                    CoreStreamingChoice newStreamingChoice = new(choiceFromSse);
                ...
                finally
                {
                    lock (_streamingChoicesLock)
                    {
                        ...
                        // hanging!) now.
                        foreach (CoreStreamingChoice streamingChoice in _streamingChoices)
                        {
        ...
        public async IAsyncEnumerable<CoreStreamingChoice> GetChoicesStreaming(
            [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            for (int i = 0; !isFinalIndex && !cancellationToken.IsCancellationRequested; i++)
            {
                ...
                CoreStreamingChoice newChoice = null;
                lock (_streamingChoicesLock)
                {
        ...
        internal CoreStreamingCompletions(
            CoreCompletions baseCompletions = null,
            IList<CoreStreamingChoice> streamingChoices = null)
        {
