...
{
    public class CoreStreamingChoice
    {
        private readonly IList<CoreChoice> _baseChoices;
        ...
        /// </summary>
        public CoreCompletionsLogProbabilityModel LogProbabilityModel
            => GetLocked(() => _baseChoices.Last().LogProbabilityModel);

        internal CoreStreamingChoice(CoreChoice originalBaseChoice)
        {
            _baseChoices = new List<CoreChoice>() { originalBaseChoice };
            _updateAvailableEvent = new CoreAsyncAutoResetEvent();
        }

        internal void UpdateFromEventStreamChoice(CoreChoice streamingChoice)
        {
